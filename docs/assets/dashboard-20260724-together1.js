...
function renderBins(item) {
  ...
  return `
    <p class="detail-section-label">All published performance tiers</p>
    ${
      item.bins.some((bin) => bin.isBelowProficiency)
        ? `
          <p class="bin-guide" aria-label="Performance tier color key">
            <span><i class="bin-key included"></i>Included in together metric</span>
            <span><i class="bin-key lowest"></i>Lowest tier</span>
          </p>
        `
        : ""
    }
    <div class="bin-list">
      ${item.bins
        .map(
          (bin) => `
            <div class="bin-row ${bin.isBelowProficiency ? "is-below-proficiency" : ""} ${
              bin.isBelowBasicAnalog ? "is-analog" : ""
            }">
...
function renderBelowProficiencyBreakdown(item) {
  if (!item.hasBelowProficiencyBreakdown) {
    return `
      <div class="breakdown-unavailable">
        <strong>Lower-tier split not published</strong>
        <span>
          This source provides a combined non-proficient result or a proficiency rate rather
          than separate additive lower tiers. Its original published view remains below.
        </span>
      </div>
    `;
  }
  ...
  return `
    <div class="proficiency-breakdown">
      <p class="breakdown-heading">What makes up the together metric</p>
      <div class="breakdown-equation">
        <div class="breakdown-piece lowest">
          <span>Lowest published tier</span>
          <strong>${formatNumber(item.analogValue)}%</strong>
          <small>${escapeHtml(item.analogLabel)}</small>
        </div>
        <span class="breakdown-operator" aria-hidden="true">+</span>
        <div class="breakdown-piece additional">
          <span>Other below proficiency</span>
          <strong>${formatNumber(item.otherBelowProficientValue)}%</strong>
          <small>${escapeHtml(item.otherBelowProficientLabel)}</small>
        </div>
      </div>
    </div>
  `;
...
function renderDetail() {
  ...
  elements.detail.innerHTML = `
    ...
    <div class="detail-result">
      <p class="detail-result-value">${formatNumber(item.notProficientValue)}<span>%</span></p>
      <p class="detail-result-label">
        <strong>Together · ${escapeHtml(item.notProficientLabel)}</strong>
        All students below the proficiency standard set by the state
      </p>
    </div>

    ${renderBelowProficiencyBreakdown(item)}
    ${renderBins(item)}
    ${naepContext}
    ...
    .map(
      (item) => `
        <tr>
          ...
          <td class="reported-measure-cell">
            <strong>${escapeHtml(item.analogLabel)}</strong>
            <span>
              ${
                item.hasBelowProficiencyBreakdown
                  ? `${formatNumber(item.analogValue)}% in lowest tier`
                  : "Published source basis"
              }
            </span>
          </td>
          <td class="numeric result-cell">${formatNumber(item.notProficientValue)}%</td>
          <td>
            <button
...
function downloadFilteredCsv() {
  ...
  const rows = [
    [
      ...
      "lowest_published_tier_or_source_basis",
      "lowest_published_tier_pct",
      "official_source",
    ],
    ...items.map((item) => [
      item.code,
      ...
      item.assessment,
      item.schoolYear,
      item.notProficientValue,
      item.proficiencyValue,
      item.analogLabel,
      item.analogValue,
      item.sourcePageUrl,
    ]),
  ...
  document.querySelectorAll("[data-sort]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (state.sortKey === nextKey) {
        ...
        state.sortDirection = nextKey === "notProficientValue" ? "desc" : "asc";
      }
      renderTable();
    });
  });
...
async function initialize() {
  try {
    const response = await fetch("data/dashboard-20260724-together1.json");
    if (!response.ok) {
    ...
    if (
      ...
      state.data.states.some(
        (item) =>
          item.notProficientValue === null ||
          item.notProficientValue === undefined ||
          item.proficiencyValue === null ||
          item.proficiencyValue === undefined ||
          item.naepValue === null ||
      ...
      throw new Error("The dashboard data bundle is incomplete.");
    }
    updateSummary();
    updateNaepSummary();
    renderExplorer();
